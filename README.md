...
- 🌳 **Two upgrade trees** — a shared **Skill tree** of character stats (Vitality, Swift Paws, Keen Nose, Soft Steps), with points earned by **clearing levels**, and a per-dog **Mastery tree** that ranks abilities up, with a point earned **every 5th level**. Open Skills with **K**, the 🌳 button, or from the inventory; respec freely to experiment.
- ⚡ **Abilities awaken at the first boss** — your dog's two active abilities (**Q / E**) start **dormant**; clearing the first biome's boss awakens them at their gentlest **level 0** tier, and mastery points (every 5 levels) rank them up through levels 1–3. Until then the early game is pure exploration — and since only abilities deal damage, that first badger simply can't be beaten, so the boss isn't about a fight. Each dog carries a distinct pair, plus a reserved **Ultimate** slot on **R**:
...
- 📜 **NPC quests & shops** — quest-givers show a glowing **"!"**; press the action key to hear the task, accept it, and hand in the goal for a reward. Tasks come in flavors — *bring* an NPC some items, or *greet* the friendly wildlife (the meadow has cozy "say hello to the ducks/squirrels" quests) — and are all optional side-tasks, so they never block clearing a level. Tailor NPCs also sell wearables and 🗝️ chest keys for treats (smart dogs pay less).
...
- 💡 **First-time feature tips** — the first time you meet a mechanic (collecting treats, cheering a friend, digging a chest, a shop, a quest, an enemy, an ability, wearing gear, leveling up, the journey map…) a little popup explains it, then never shows again. Some carry a **pixel-art illustration** — the treasure tip shows the loose-dirt dig spot and each chest tier so you know what to look for. Each notes it can be turned off, and **Options** has a toggle plus "show all tips again."
